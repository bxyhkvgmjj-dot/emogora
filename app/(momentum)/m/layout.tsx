            ...
            <nav className="mt-8 flex flex-col gap-2 text-sm">
              {nav.map((n) => (
                <Link
                  key={n.href}
                  href={n.href}
                  className="rounded-xl px-3 py-2 text-zinc-700 hover:bg-white/50 hover:text-zinc-900 transition"
                >
                  {n.label}
                </Link>
              ))}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <header className="sticky top-0 z-30 border-b border-white/40 bg-white/30 backdrop-blur-md">
            <div className="flex items-center justify-between px-4 py-3 md:px-8">
              ...
              <div className="flex items-center gap-2">
                <button className="rounded-2xl border border-white/60 bg-white/50 px-3 py-2 text-sm shadow-sm hover:bg-white/70 transition">
                  + New
                </button>
                <div className="h-9 w-9 rounded-full bg-white/60 border border-white/60" />
              </div>
            </div>
          </header>
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-4 px-2 py-2 text-xs">
          {nav.map((n) => (
            <Link
              key={n.href}
              href={n.href}
              className="flex flex-col items-center justify-center gap-1 rounded-xl py-2 text-zinc-700 hover:bg-white/50 transition"
            >
